...
/**
 * {@link Es256Verify} on a DER-only provider: BC-FIPS registers no raw-format ECDSA name, so verification must resolve
 * {@code SHA256withECDSA} and re-encode the raw {@code r || s} signature to minimal DER ({@code EcdsaDer.toDer}) before
 * handing it over. This is the platform the verification helper exists in the core for — a starter-local copy would
 * have shipped without this coverage, and its absence is exactly what would condemn a perfectly healthy signer on a
 * FIPS-restricted JVM to a permanent {@code DOWN} in the health probe.
 */
class BcFipsEs256VerifyTest {

    private static final byte[] SIGNING_INPUT =
            "push2u BC-FIPS Es256Verify test input".getBytes(StandardCharsets.US_ASCII);
    ...
    void verifiesRawSignaturesThroughTheDerOnlyProvider() {
        ...
        assertThat(Es256Verify.isSupported(jca))
        ...
        assertThat(Es256Verify.verify(jca, signer.publicKey(), SIGNING_INPUT, rawSignature))
        ...
        assertThat(Es256Verify.verify(jca, signer.publicKey(), SIGNING_INPUT, garbage))
