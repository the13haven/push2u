...
/**
 * {@link LocalEcVapidSigner} bound to a BC-FIPS provider instance (not registered with
 * {@code java.security.Security}) satisfies the shared {@link VapidSignerContractTest}.
 * BC-FIPS registers only DER-format ECDSA, so every signature here goes through the strict
 * DER → P1363 conversion — and the contract verifies the converted 64-byte output against a
 * platform P1363 verifier, proving the conversion preserves the signature.
 *
 * <p>Lives in the {@code fipsTest} source set: bc-fips and stock bcprov ship incompatible
 * versions of {@code org.bouncycastle.crypto.CryptoServicesRegistrar} and can never share a
 * classpath, so the BC-FIPS tests run on their own bcprov-free classpath.
 */
