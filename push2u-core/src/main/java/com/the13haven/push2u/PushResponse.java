...
/**
 * The push service's answer to a POST: the status code and the response headers. Header names are stored and looked up
 * case-insensitively, as HTTP requires.
 *
 * <p>A {@link PushHttpClient} returns this; the {@link PushSender} reads the status and {@code Retry-After}. There is
 * deliberately no body: RFC 8030 push delivery never consumes one, and materializing it would let a hostile endpoint
 * (the URL comes from the subscription) feed the sender an arbitrarily large response.
 *
...
public record PushResponse(int statusCode, Map<String, String> headers) {
    ...
    /**
     * Rejects a negative status code, then lower-cases the header keys (for case-insensitive lookup) and makes the map
     * immutable.
     *
