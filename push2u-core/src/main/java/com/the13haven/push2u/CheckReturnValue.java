 ...
 * for static analysers rather than for the compiler: an analyser that matches a mark of this <em>simple name</em>,
 * whatever package it comes from, reports a call that throws the answer away — a bare {@code someObject.assess(uri);}
 * statement, and a method reference such as {@code list.forEach(someObject::assess)} alike. Error Prone's <a
 * href="https://errorprone.info/bugpattern/CheckReturnValue">{@code CheckReturnValue}</a> check is the one this was
 * measured against: it matches by simple name, is on by default at {@code ERROR} severity, and so costs a consumer
 * already running it no configuration and no dependency. What any other analyser or IDE does with this mark is not
 * claimed here, because it was not measured.
 *
