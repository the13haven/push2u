 ...
 * control flow.
 *
 * <p>The record is public and so is its canonical constructor — callers construct one in tests
 * and fakes — so the contract the fields document is enforced rather than merely stated: a
 * {@code null} status, a negative status code or an attempt count below one would each describe
 * a send that cannot have happened.
 *
...
public record PushResult(Status status, int statusCode, int attempts) {
    ...
    public PushResult {
        ...
        if (statusCode < 0) {
            throw new IllegalArgumentException("statusCode must not be negative (0 means none was obtained), was "
                + statusCode);
        }
        if (attempts < 1) {
            throw new IllegalArgumentException("attempts must be at least 1 — a result exists only after a POST, was "
                + attempts);
        }
    }
    ...
    public enum Status {
        /**
         * A 2xx — the push service accepted the message. RFC 8030 §5 has a push service answer a
         * successful send with {@code 201 Created}, and that is what the major services return, but
         * the whole 2xx class is accepted: the RFC's own retry advice keys off 4xx/5xx, and a service
         * answering {@code 200} or {@code 202} has accepted the message just the same. The precise
         * code is in {@link PushResult#statusCode()} for a caller that needs to tell them apart.
         */
        DELIVERED,
