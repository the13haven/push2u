...
One thing is deliberately not withheld, and the line is worth stating because the endpoint policy's
contract draws it elsewhere. A subject's own exception travels as the failure's cause, so whatever
a defective transport put in its own message is printed. That is the frame an implementor has to
look at, and it costs nothing here: every value on this wire was generated by the harness — the
loopback endpoint it minted, the headers and the body it made — so what a transport can quote back
is synthetic, and no subscription exists in the run to be disclosed. The policy contract withholds
more because its witness is supplied by the implementor and may be a real endpoint.
