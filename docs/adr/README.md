...
| [021](0021-retry-belongs-to-the-caller.md) | Retry belongs to the caller | Proposed | A retry loop inside the sender, in any configuration; a retry SPI, policy interface or any seam whose subject is retrying; a retrying wrapper shipped from this repository, in the core or in a module of its own; `RetryAfter.parse` as public API; a `Retry-After` clamped before it reaches the caller; an attempt count on a result the sender no longer controls; an unanswered POST reported as safe to repeat; an interrupted send reported as a retryable result; one exception type covering a failure that recurs and one that may not; a retryable class rule in place of a status matrix; a result named for delivery when the push service reported acceptance |
