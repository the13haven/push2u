...
| [021](0021-retry-schedule-is-a-seam.md) | The retry schedule is a seam, and the caller may be the driver | Proposed | A driver or callback SPI over the retry loop; a library-defined resumable retry state or a serialization format for one; a policy that widens the retryable status set; a public type for the built-in exponential schedule; `RetryAfter.parse` as public API; a `Retry-After` clamped or filtered by status before it reaches the caller; an endpoint or subscription inside the retry context; a library-imposed ceiling over a replaced policy; a consumer's policy defect reported as a delivery or crypto failure; `Sleeper` as a public seam; a property spelling for a replaced policy, or a properties-versus-bean exclusivity the defaulted keys cannot express; a `PushResult` component outside its canonical constructor |
