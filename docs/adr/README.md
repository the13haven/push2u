...
| [023](0023-one-size-limit-answerable-before-a-send.md) | One configured size limit, answerable before a send | Proposed | Two configurable size parameters, or any second knob derivable from the first; `recordSize` as public API in its role as a refusal threshold, in the builder or as a Spring property that configures anything — a padding width being a different decision this one does not foreclose; `rs` derived per message rather than per sender; a floor, default or historical constant applied to the derived `rs`; a degenerate `min` left in the pre-flight after its second operand became derivable; a hardcoded plaintext maximum, or a plaintext limit as the configured knob; a bare numeric budget on the public API, in any spelling that lets a caller reach it without one guarded comparison first; a pre-flight answer typed as a `PushOutcome`, or an outcome variant reused to answer a question that sent nothing; a derived component published on an assessment because it shortens a log line; a pre-flight that replaces the refusal, or a `send` that trusts an earlier assessment |
