...
| [021](0021-retry-belongs-to-the-caller.md) | Retry belongs to the caller | Proposed | A retry loop inside the sender; a retry SPI, policy interface or any seam whose subject is retrying; a retrying wrapper shipped from this repository, in the core or in a module of its own; a library-defined resumable retry state or a serialization format for one; `RetryAfter.parse` as public API; a `Retry-After` clamped before it reaches the caller; an attempt count on a result the sender no longer controls; a result field whose meaning depends on which outcome it is attached to; one exception type covering a failure that recurs and one that may not |
