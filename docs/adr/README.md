...
| [018](0018-encoded-vapid-public-key-on-the-signer.md) | The encoded VAPID public key is part of the signer contract | Proposed | An abstract addition to `VapidSigner`; a general-purpose base64url codec published from the core; an encoder for the private scalar; a padded or configurable encoding; an on-curve check inside the `default` method, and a merely structural one inside the static |
