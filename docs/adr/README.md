...
| [024](0024-one-endpoint-policy-reachable-at-registration.md) | One endpoint policy, reachable where subscriptions are accepted | Proposed | An endpoint policy unreachable at the boundary where subscriptions are accepted; a second policy instance built by the application from properties another module owns; an accessor for the policy on `PushSender`; a second method on `EndpointPolicy` — a predicate, or an overload taking a `Subscription`; a core factory joining subscription parsing to the deployment's admission decision, or a library-owned registration boundary; a policy built inside an auto-configuration's sender factory and left unrepresented in the context; an application-supplied policy bean winning silently over a non-empty allowlist property; a policy bean in a deployment that configured no signer, and the startup failure that would come with it; a registration check a later send trusts, or one offered as a replacement for the send's check; a configuration-only path to unrestricted egress |
