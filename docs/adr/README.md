...
| [022](0022-one-type-per-remediation.md) | One exception type per remediation | Proposed | A type per throw site; a type whose only consumer difference is the sentence it puts in a log; `IllegalArgumentException` as a base class for a type the library owns, or as the carrier of library semantics; a library exception not extending `RuntimeException` directly; message-text matching as the supported way to tell two conditions apart; a transient condition and a permanent one sharing a type; a cancellation wearing the type of whatever it interrupted; a signer's retry hint modelled as sometimes-absent where one half can never carry it; a taxonomy settled one exception at a time in whichever ADR touches a failure next |
