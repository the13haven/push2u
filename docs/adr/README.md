...
| [019](0019-vapid-token-reused-until-it-nears-expiry.md) | The VAPID token is reused until it nears expiry | Proposed | A token cache behind an SPI; a shared cache level in front of the in-process one; an unbounded or unevictable cache; a proportional safety margin; a second spelling of "sign every time"; a signature taken under the cache's lock; a cached entry surviving a backwards clock step; a cache invalidated by an authentication status; a token bounded by the signing key's life or naming more than one origin in `aud`; a `byte[]` cache key; a claim about a named push service accepting a reused token |
