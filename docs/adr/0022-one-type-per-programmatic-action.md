...
**Status:** Accepted; one clause superseded by [ADR-027](0027-the-endpoint-policy-answers-with-a-value.md)
...
| It answered `404` or `410` | the response | `SubscriptionExpired(statusCode)` | Stop sending to it, and delete it wherever it is stored — the library stores none |
| It answered `408`, `421`, `429`, a `413` carrying a parseable `Retry-After`, or a 5xx the matrix does not carve out | the response | `RetryableFailure(statusCode, retryAfter)` | Decide whether the duplicate risk is acceptable; if repeating, schedule it not before the hint where one arrived, and on a `507` not before a fresh user action where the send was one |
...
| The custodian cannot sign now — a state of the custodian rather than of what the deployment configured: unreachable, sealed, not initialized, not caught up, rate-limited | `VapidSigner` throws `VapidSignerUnavailableException`, carrying the cause, the status wherever the custodian answered a number, and a hint only where one was declared | `SignerUnavailable` | Stop submitting new sends; repeat when the custodian is back, and not before a hint it declared. Nothing was sent, so no repeat can duplicate |
