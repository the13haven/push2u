...
final class PushHttpClientContractSelfTest {
    ...
    /**
     * The line this contract draws around its own output, pinned from both sides so that neither half is "fixed" later
     * on its own. The failure message names what did not match and no values; the subject's exception is kept as the
     * cause, because it is the frame an implementor has to look at and everything it can quote back was generated by
     * the harness rather than taken from a subscription.
     */
    @Test
    void aFailureNamesNoValuesAndStillCarriesTheSubjectsOwnException() {
        Contract contract = Contract.over((ssl, trust) -> (endpoint, headers, body) -> {
            throw new IllegalStateException("thrown by the subject about " + endpoint);
        });
