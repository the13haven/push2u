...
public abstract class PushHttpClientContractTest {
    ...
    // values. What the contract does not withhold is the subject's own exception, which travels as
    // the cause: it is the frame an implementor has to look at, and everything on this wire was
    // generated by the harness — a loopback endpoint it minted, headers and a body it made — so
    // what a transport can quote back is the harness's synthetic material and never a
    // subscription. The policy contract in this package holds the stricter line for the opposite
    // reason: the endpoint it works with is supplied by the implementor and may be a real one.
