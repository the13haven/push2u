...
public abstract class PushHttpClientContractTest {
    ...
    // library, because an assertion prints its actual value — a response or an exception a
    // defective transport may have built out of anything at all — and this contract's failures go
    // to a consumer's build log. The messages here and below name what did not match and no
    // values. One thing is deliberately not withheld and the boundary is worth stating exactly: a
    // subject's own exception travels as the cause, and a test runner prints a cause with whatever
    // text the subject put in it. That text is not necessarily harmless — a transport adds headers
    // out of the environment it runs in, a proxy credential being the usual example, and nothing
    // stops one from quoting its own configuration in a message. It is kept because it is the
    // frame an implementor has to look at, and because it is their own exception in their own
    // build. The line every contract in this package holds is the one about what the kit itself
    // renders: never a value it was given or observed.
    ...
    void theResponseHeadersReachTheCaller() throws Exception {
        try (TransportContractServer server =
            ...
            // The one-request obligation, asserted here because this is the answer a repeat is
            // most likely to be conditioned on. A transport retrying only when the push service
            // says "come back later" satisfies the check below, whose scenario answers 201, and
            // still delivers the notification twice in production.
            requireOneRequest(server, "a 429, the answer a repeat is likeliest to be conditioned on");
        }
    }
    ...
    void anUnansweredRequestIsADeliveryFailure() throws Exception {
        try (TransportContractServer server = TransportContractServer.answeringWith(request -> Optional.empty())) {
            ...
            // transport retrying after an unanswered exchange has already delivered the message
            // once for all it knows, and the exception it finally raises tells the sender that
            // nothing was delivered at all.
            requireOneRequest(server, "nothing at all");
        }
    }
    ...
    /**
     * The one-request obligation, in whichever scenario a check needs it. A transport neither classifies nor repeats: a
     * repeat inside one {@code post} delivers the notification twice while the sender it reports one outcome to never
     * learns of the second. It is asserted in three scenarios rather than one, because the two answers a real repeat is
     * conditioned on — a status the push service asks to be retried, and an exchange nothing answered — are invisible
     * to a scenario that answers success, and those two are where a repeating transport actually repeats.
     *
     * @param server the harness whose record of arrivals is read
     * @param answer what the harness answered, named in the failure so the scenario is identifiable
     * @return the one request that arrived, for a caller that goes on to inspect it
     */
    private static TransportContractServer.ReceivedRequest requireOneRequest(
            TransportContractServer server, String answer) {
        ...
        if (received.size() != 1) {
            throw new AssertionError("exactly one request must arrive per post call — the seam says a transport "
                    + "neither classifies nor repeats, and a repeat delivers the notification twice while the sender "
                    + "learns of one outcome. Answering " + answer + ", the server saw " + received.size()
                    + " requests.");
        }
