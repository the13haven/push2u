...
`https://gw.example/vault` and `https://gw.example/vault/` therefore address the same Vault, and
`https://vault.example:8200` keeps its historical shape. The key name and the token travel as the value types
