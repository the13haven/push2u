...
cache misses, plus a floor of health probes a bean-level decorator cannot separate from sends. Its
...
how to use it — and `docs/adr/` holds the decisions behind it, one file per ADR, numbered from 001,
with `docs/adr/README.md` as the index. **No count of them is kept here**, and the omission is
deliberate: the index is where a new record reads its next free number, and a total written in a
second place is wrong precisely when someone uses it for that. Read the relevant ADR before changing
anything structural.
